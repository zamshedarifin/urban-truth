                            ...
                            <div id="customerList">
                                ...
                                <div class="table-responsive table-card mt-3 mb-1">
                                    <table class="table align-middle table-nowrap" id="customerTable">
                                        ...
                                        @forelse($childCategories as $k=>$subCategory)
                                            <tr>
                                                <td>{{$k+1}}</td>
                                                    ...
                                                    <div class="d-flex gap-2">
                                                        <div class="edit">
                                                            <button class="btn btn-sm btn-success edit-item-btn"
                                                                    ...
                                                                    data-category="{{ $subCategory->cat_id }}"
                                                                    data-sub_category="{{ $subCategory->sub_cat_id }}"
                                                                    data-title="{{ $subCategory->title }}"
                                                                    data-id="{{ $subCategory->id }}">Edit
                                                            </button>
                                                        </div>
                                                    </div>
                                                </td>
    ...
    <div class="modal fade" id="showModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                ...
                <form action="{{route('admin.child-category')}}" method="post">
                    @csrf
                    <div class="modal-body">
                        ...
                        <div class="mb-3" id="modal-id">
                            <label for="id-field" class="form-label">Select Sub-Category</label>
                        ...
                        <div class="mb-3" id="modal-id">
                            <label for="id-field" class="form-label">Child-Category Title</label>
                            <input type="text" id="id-field" class="form-control  @error('title') is-invalid @enderror"
                                   placeholder="Sub-category Title" name="title" value="{{old('title')}}"/>
                            @error('title')
                            <span class="text-danger">{{ $message }}</span>
                            @enderror
                        </div>
                ...
                <form action="{{route('admin.child-category.update')}}" method="post">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="mb-3" id="modal-id">
                            <label for="id-field" class="form-label">Select Sub-Category</label>
                            <select class="form-control  @error('sub_category') is-invalid @enderror"
                                    name="sub_category" id="subCatgory">
                                @foreach($subCategories as $sub_cat)
                                    <option value="{{$sub_cat->id}}"
                                            @if(old('sub_category') == $sub_cat->id ) selected @endif>{{$sub_cat->title}}</option>
                                @endforeach
                            </select>
                            @error('sub_category')
                            <span class="text-danger">{{ $message }}</span>
...
@push('js')
            ...
            $("#categoryEdit").change(function () {
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        ...
                        // $('#CityList').html(html);
                        $('#subCatgory').empty();
                        $('#subCatgory').append('<option value="">Select Category</option>');
                        $.each(data, function (index, supproobj) {
                            $('#subCatgory').append('<option value="' + supproobj.id + '">' + supproobj.title + '</option>');
                        });
                    }
                });
            });
    ...
    <script>
        $('.edit-item-btn').on('click', function() {
            ...
            modal.find('select[name=sub_category]').val($(this).data('sub_category'));
